...
import { UserDocModel } from "./userdocs.model.js";
...
export async function uploadUserDoc(req: Request, res: Response) {
  try {
    ...
    const doc = await UserDocModel.create({
      userId,
      title,
      description:
...
export async function listUserDocs(req: Request, res: Response) {
  try {
    ...
    const docs = await UserDocModel.find({ userId })
      .sort({ createdAt: -1 })
      .lean();
...
export async function updateUserDoc(req: Request, res: Response) {
  try {
    ...
    const { id } = req.params;
    const doc = await UserDocModel.findOne({ _id: id, userId });
